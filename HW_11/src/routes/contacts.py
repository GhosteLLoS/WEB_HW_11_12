...
async def read_contacts(filter_: str = None, db: AsyncSession = Depends(get_db),
                        current_user: User = Depends(auth_service.get_current_user)):
    if filter is None:
        contacts = await repository_contacts.get_contacts(current_user, db)
        return contacts
    contacts = await repository_contacts.find_contacts(filter_, current_user, db)
    if not contacts:
...
async def read_birthdays(db: AsyncSession = Depends(get_db),
                         current_user: User = Depends(auth_service.get_current_user)):
